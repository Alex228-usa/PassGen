...
class PassGen:
    ...
    def question(self, target):
        ...
        while True:
            bday = self.prompt('Введите дату рождения {}(дд.мм.гггг / 04.04.2019): '.format(target))

            if not len(bday.strip()):
                break

            if len(bday.split('.')) != 3:
                print('Неправельный формат даты ДР\n')
                continue
            
            for _ in bday.split('.'):
                if not _.isdigit():
                    print('Формат принимает только цифры\n')
                    continue
    ...
    def format_names(self):
        for _ in range(self.max_count_main):
            if not self.silent:
                print(f'Сгенерировано паролей: {len(self.passwords)}')
    ...
    def generator(self, ignore_additional = True):
        ...
        self.spouse = self.question('жены/мужа')
        print('\n')

        self.child = self.question('ребёнка')
        print('\n')

        self.pet = self.question('питомца')
        print('\n')

        print('Генерация возможных паролей... \n Это может занять некоторое время.')
        self.format_names()
        if self.silent:
            print("...сгенерировано {} возможных паролей".format(len(self.passwords)))
        ...
        with open(output_file, 'wt') as f:
            for pwd in self.passwords:
                if not self.silent:
                    print('Записиваем пароли ...')
                f.write('{}\n'.format(pwd))

        if not ignore_additional:
            print("Генерация дополнительных паролей")
            with open(output_file, 'at') as f:
                i = 0
                while(i < self.max_count):
                    if not self.silent:
                        print('Записиваем дополнительные пароли ... {}/{}'.format(i*3, self.max_count*3))
                    f.write('{}{}\n'.format(self.target['firstname'], i))
                    f.write('{}{}\n'.format(self.target['lastname'], i))
                    f.write('{}{}\n'.format(self.target['nickname'], i))
                    i += 1
            if self.silent:
                print("...сгенерировано {} возможных паролей".format(self.max_count*3))

        print('Паролей сгенерировано в файле: {}'.format(output_file))
        quit()
